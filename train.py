...
from trainer import BaseTrainer,FITrainer,ADMMTrainer,UndercoverTrainer,RepNoiseTrainer,LDIFSTrainer, UnitedTrainer,VlguardTrainer,UnitedAlignmentTrainer,BoosterAlignmentTrainer, Panacea, SFT_eval
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer, data_args, training_args) -> Dict:
    ...
    if "BeaverTails_safe" not in data_args.data_path:
        ...
        # eval_dataset=SupervisedDataset(tokenizer=tokenizer, data_path=data_args.data_path, poison_ratio=1,sample_num=1000, benign_dataset=data_args.benign_dataset,poison_data_start=5000)
    else:
        eval_dataset=SupervisedDataset(tokenizer=tokenizer, data_path=data_args.data_path, poison_ratio=1,sample_num=1000, benign_dataset=data_args.benign_dataset,poison_data_start=5000)
...
def train():
    ...
    parser.add_argument("--eps_rho",  type=float, default=1.0, help="Specify the optimizer to use")
    parser.add_argument("--eps_lr",  type=float, default=1e-4, help="Specify the optimizer to use")
    parser.add_argument("--log_dir",  type=str, default="", help="Specify the optimizer to use")
    parser.add_argument("--tag",  type=str, default="no_eps", help="Specify the optimizer to use") 
    parser.add_argument("--add_eps",  type=str, default="True", help="Specify the optimizer to use") 
    parser.add_argument("--noise_std",  type=float, default=0, help="Specify the optimizer to use")
    ...
    training_args.eps_lr = extra_args.eps_lr
    training_args.eps_rho = extra_args.eps_rho
    ...
    if extra_args.log_dir:
        writer = SummaryWriter(log_dir=extra_args.log_dir)
    ...
    if training_args.optimizer=="vaccine":
        ...
        harmful_dataset  = SupervisedDataset(tokenizer=tokenizer,data_path="BeaverTails_dangerous", poison_ratio=1,sample_num=data_args.bad_sample_num,benign_dataset=data_args.benign_dataset,poison_data_start=5000)
        trainer = Pancacea(model=model, tokenizer=tokenizer, args=training_args ,**data_module)
        trainer.init(harmful_dataset, model, writer, extra_args.tag)
        ...
        # trainer = SFT_eval(model=model, tokenizer=tokenizer, args=training_args ,**data_module)
        # trainer.init(std=extra_args.noise_std)
    ...
    print("add eps:",extra_args.add_eps)
    if extra_args.tag == "eps" and extra_args.add_eps == "True":
