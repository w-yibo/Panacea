...
class SFT_eval(Trainer):
    ...
    def evaluate(
        ...
        self.eval_metric.append(metrics)
        return metrics
    def get_eval_metric(self):
        return self.eval_metric

class Panacea(BoosterAlignmentTrainer):
    def init(self, harmful_datast, model, writer, tag):
        self.clock = 0
        self.steps = 0
        if self.args.guide_data_num>0:
            self.harmful_dataloader = self.get_harmful_dataloader(harmful_datast)
    ...
    def training_step(
        ...
        def step_eps():
            ...
            self.writer.add_scalar("Loss/fine_tuning_loss", loss_g, self.steps)
            self.writer.add_scalar("Loss/harmful_loss_h+eps", loss_h, self.steps)
    ...
    #     metric_key_prefix: str = "eval",
    # ) -> Dict[str, float]:
    #     print(f"Starting panacea evaluation")
    #     for name, param in self.model.named_parameters():
    #         if param.requires_grad:
    #             param.data += self.epsilon[name]       
    #     metrics = super().evaluate(
    ...
    #         if param.requires_grad:
    #             param.data -= self.epsilon[name]
    #     print(f"finish panacea evaluation")
    #     self.eval_metric.append(metrics)
    #     return metrics
    # def get_eval_metric(self):
    #     return self.eval_metric
